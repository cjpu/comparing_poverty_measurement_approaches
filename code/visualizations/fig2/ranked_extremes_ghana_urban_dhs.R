...
# Purpose: Visualize accordance between "bottom 10" (i.e., households with the lowest 10 DHS scores) and "top 10" (i.e., households with the highest 10 DHS scores) households based on their PPI scores, DHS scores, regular expenditures per capita, Krishna's Stages of Progress in urban Ghana
# Inputs: Cleaned dataset (ghana_listing_selected_vars_cleaned.rds) produced by clean_ghana.R
# Outputs: Figure 2
# Sample sizes: Urban Wassa East (n = 209), Urban Asutifi North (n = 640)
...
# defining top 10 and bottom 10 ranks for urban asutifi north households only ----------------------------------------------

t10_dhs_urban_asutifinorth = 631
b10_dhs_urban_asutifinorth = 10
t10_ppi_urban_asutifinorth = 596
b10_ppi_urban_asutifinorth = 8
t10_regexppcpd_urban_asutifinorth = 630
b10_regexppcpd_urban_asutifinorth = 10

# comparing top 10 and bottom 10 dhs households in urban wassa east------------------------------------------------------------------
...
ghana_listing_cleaned %>% 
  ...
  filter(
    dhs_rank_rounded <= b10_dhs_urban_wassaeast
  ) %>% 
...
# comparing top 10 and bottom 10 dhs households in urban asutifi north ------------------------------------------------------------------
...
ghana_listing_cleaned %>% 
  ...
  filter(
    dhs_rank_rounded <= b10_dhs_urban_asutifinorth
  ) %>% 
    ...
    list(
      filler1 = formatter("span",
                          style = ~style(display = "block",
                                         ...
                                         "padding-right" = "4px",
                                         "background-color" = 
                                           ifelse(filler1 <= b10_dhs_urban_asutifinorth, "red",
                                                  ifelse(filler1 >= t10_dhs_urban_asutifinorth, "green", "lightgrey"
                                                  )
                                           )
                          )
      ),
      filler2 = formatter("span",
                          style = ~style(display = "block",
                                         ...
                                         "padding-right" = "4px",
                                         "background-color" = 
                                           ifelse(filler2 <= b10_ppi_urban_asutifinorth, "red",
                                                  ifelse(filler2 >= t10_ppi_urban_asutifinorth, "green", "lightgrey"
                                                  )
                                           )
                          )
      ),
      filler3 = formatter("span",
                          style = ~style(display = "block",
                                         ...
                                         "padding-right" = "4px",
                                         "background-color" = 
                                           ifelse(filler3 <= b10_regexppcpd_urban_asutifinorth, "red",
                                                  ifelse(filler3 >= t10_regexppcpd_urban_asutifinorth, "green", "lightgrey"
                                                  )
                                           )
                          )
      ),
