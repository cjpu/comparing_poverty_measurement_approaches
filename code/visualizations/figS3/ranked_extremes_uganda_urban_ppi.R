...
# Purpose: Visualize accordance between "bottom 10" (i.e., households with the lowest 10 PPI scores) and "top 10" (i.e., households with the highest 10 PPI scores) households based on their PPI scores, DHS scores, regular expenditures per capita, Krishna's Stages of Progress in urban Uganda
# Inputs: Cleaned dataset (uganda_listing_selected_vars_cleaned.rds) produced by clean_uganda.R
# Outputs: Supplementary Figure 3
# Sample sizes: Urban Kabarole (n = 519) 
...
# comparing top 10 and bottom 10 ppi households in urban kabarole ------------------------------------------------------------------
...
uganda_listing_cleaned %>% 
  ...
  filter(
    ppi_rank_rounded <= b10_ppi_urban_kabarole
  ) %>% 
